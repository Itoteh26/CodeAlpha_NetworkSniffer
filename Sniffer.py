...
# ==============================================================================
# STEP 1: GLOBAL SNIFFER CONFIGURATION
# ==============================================================================
TARGET_INTERFACE = None
BPF_FILTER = ""
PACKET_COUNT = 0
...
# ==============================================================================
# STEP 2: THE DATA DISSECTOR (CALLBACK FUNCTION)
# ==============================================================================
def analyze_packet(packet):
    ...
    if packet.haslayer(IP):
        ...
        # Extract and map the Protocol number
        proto_num = packet[IP].proto
        protocol = "UNKNOWN"
        ...
        # Initialize raw port strings
        src_port = ""
        dst_port = ""

        # --- LAYER 4: TRANSPORT CONTROL (TCP / UDP / ICMP) ---
        if packet.haslayer(TCP):
            src_port = str(packet[TCP].sport)
            dst_port = str(packet[TCP].dport)
        ...
        if packet.haslayer(Raw):
            raw_bytes = packet[Raw].load
            payload_data = raw_bytes.decode('utf-8', errors='ignore').strip()
            
            # Keep payload single-lined and truncated for display neatness
            payload_data = payload_data.replace('\n', ' ').replace('\r', '')
            if len(payload_data) > 80:
                payload_data = payload_data[:80] + "..."
        
        if not payload_data:
            payload_data = "No Payload"

        # --- DISPLAY LOG ---
        # Formatted using clean column alignment matching your screenshot
        print("=========================================")
        print(f"Time Stamp       : {current_time}")
        print(f"Source IP        : {src_ip}")
        print(f"Destination IP   : {dst_ip}")
...
# ==============================================================================
# STEP 3: THE SNIFFER ENGINE MAIN LOOP
# ==============================================================================
def main():
    print("==========================================================")
    print("        CodeAlpha Network Packet Sniffer Active           ")
    ...
    try:
        ...
        print("\n[-] Sniffer terminated by user. Exiting cleanly...")
    except PermissionError:
        print("\n[!] ERROR: Permission Denied!")
        print("[!] Packet sniffing requires Administrative/Root privileges.")
        print("[!] Please rerun Git Bash as an Administrator.")


# ==============================================================================
# STEP 4: TRIGGER PROGRAM EXECUTION
# ==============================================================================
if __name__ == "__main__":
    main()